#!/usr/bin/env Rscript
# Run the original R package scCDC end-to-end on an external counts matrix
# (genes x cells TSV, the same orientation the Python side dumps via
# pandas.to_csv) plus a per-cell cluster table.  Outputs the GCG list, the
# per-gene/per-cluster entropy, the entropy-divergence (distance) table,
# the per-GCG Youden subtraction thresholds, the corrected count matrix
# and the dataset contamination ratio — everything the comparison
# notebook needs to overlay onto the pysccdc results.
#
# Usage:
#   Rscript r_driver_sccdc.R <counts_tsv> <clusters_csv> <outdir>
#
# Inputs:
#   counts_tsv    genes x cells integer count matrix (row 1 = cell ids,
#                 col 1 = gene ids)
#   clusters_csv  two columns: cell, cluster
#
# Outputs (in outdir):
#   entropy.csv     per-gene, per-cluster Shannon entropy of counts
#   detection.csv   ContaminationDetection() degree-of-contamination table
...
counts <- as.matrix(read.table(counts_tsv, sep = "\t", header = TRUE,
...
cat("[R] running ContaminationDetection()\n")
...
cat("[R] running ContaminationCorrection()\n")
...
cat(sprintf("[R] wrote outputs to %s\n", out_dir))
